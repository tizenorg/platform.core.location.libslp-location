...
int main(int argc, char** argv)
{
	...
	while(1)
	{
		...
		if (0 == g_strcmp0 ("x", strOpt)) {
			...
			else if (opt == 1) add = location_address_new (NULL, "삼성로", "매탄3동", "수원시 영통구", "경기도", NULL, NULL);
			...
			else if (opt == 1) data->addr = location_address_new (NULL, "삼성로", "매탄3동", "수원시 영통구", "경기도", NULL, NULL);
			...
			if(opt == 0){
				...
				addr = g_strdup("대한민국 경기도 수원시 영통구 매탄동 436-52");
			}else if(opt == 2){
			...
			if(opt == 0){
				...
				data->str_addr = g_strdup("대한민국 경기도 수원시 영통구 매탄동 436-52");
			}else if(opt == 2){
